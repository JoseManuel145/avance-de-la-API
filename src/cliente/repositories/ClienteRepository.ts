...
export class ClienteRepository {
    ...
    public static async findById(cliente_id: number): Promise<Cliente | null> {
        return new Promise((resolve, reject) => {
            connection.query('SELECT * FROM clientes WHERE cliente_id = ?', [cliente_id], (error: any, results) => {
                if (error) {
                    reject(error);
                } else {
                    const clientes: Cliente[] = results as Cliente[];
    ...
    public static async findByEmail(email: string): Promise<Cliente | null> {
        return new Promise((resolve, reject) => {
            connection.query('SELECT * FROM clientes WHERE email = ?', [email], (error: any, results) => {
                if (error) {
                    reject(error);
                } else {
                    const clientes: Cliente[] = results as Cliente[];
    ...
    public static async createCliente(cliente: Cliente): Promise<Cliente> {
        const query = 'INSERT INTO clientes (email, full_name, password, created_by) VALUES (?, ?, ?, ?)';
        return new Promise((resolve, reject) => {
    ...
    public static async updateCliente(cliente_id: number, clientData: Cliente): Promise<Cliente | null> {
        const query = 'UPDATE clientes SET full_name = ?, password = ?, updated_at = ?, updated_by = ? WHERE cliente_id = ?';
        return new Promise((resolve, reject) => {
            connection.execute(query, [
